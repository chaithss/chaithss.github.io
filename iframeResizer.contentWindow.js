...
(function (undefined) {
  ...
  function init() {
    ...
    readyCallback();
    window.parentIFrame.sendMessage({event:"navigation","currentURL" : window.location.href});
  }
  ...
  function manageTriggerEvent(options) {
    ...
    var listener = {
      add: function (eventName) {
        ...
        if('custom' === options.eventType){
          var buttonElements = document.getElementsByTagName('button');
          for(var buttonElement of buttonElements){
            addEventListener(buttonElement, eventName, handleEvent);
          }

          var anchorElements = document.getElementsByTagName('a');
          for(var anchorElement of anchorElements){
            addEventListener(anchorElement, eventName, handleEvent);
          }
        }else{
  ...
  function manageEventListeners(method) {
    manageTriggerEvent({ method: method, eventType: 'custom', eventName: 'click' });
    ...
    function recordTrigger() {
      if (!(triggerEvent in { 'reset': 1, 'resetPage': 1, 'init': 1 })) {
        ...
        if ('resizeParent' !== triggerEvent) {
          window.parentIFrame.sendMessage({"event": triggerEvent, "description": triggerEventDesc, "href": event.target.href});
        }
      }
    }
